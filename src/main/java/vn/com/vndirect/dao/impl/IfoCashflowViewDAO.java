...
import vn.com.vndirect.domain.IfoCashflowViewId;
...
/**
 * Data access object (DAO) for domain model class IfoCashflowView.
 * 
 * @see vn.com.vndirect.domain.IfoCashflowView
 * @author MyEclipse - Hibernate Tools
 */
@SuppressWarnings({ "unchecked", "rawtypes" })
public class IfoCashflowViewDAO extends BaseDAOImpl {

	private static final Log log = LogFactory.getLog(IfoCashflowViewDAO.class);

	/**
	 * process all attributes of table
	 */
	private final RowMapper ifoCashflowViewRowMapper = new RowMapper() {
		public Object mapRow(ResultSet rs, int arg1) throws SQLException {
			IfoCashflowViewExt obj = new IfoCashflowViewExt();
			IfoCashflowViewId objId = new IfoCashflowViewId();
	...
	public List findByCurrentCompany(IfoBalanceSheetSearch searchObject) throws SystemException {
		...
		try {
			...
			sql.append("SELECT distinct t0.* FROM IFO_CASHFLOW_VIEW t0 ");
			...
			SearchResult result = OracleDAOHelper.query(this.getDataSource(), sql.toString(), paramMap, ifoCashflowViewRowMapper,
			        null);
	...
	public List fiscalYearList(IfoBalanceSheetSearch searchObject) throws SystemException {
		...
		try {
			Map paramMap = new HashMap();
			StringBuffer sql = new StringBuffer(
			        "SELECT DISTINCT TO_CHAR(FISCAL_DATE, 'YYYY') AS FISCAL_YEAR FROM IFO_CASHFLOW_VIEW WHERE COMPANY_ID = :COMPANY_ID  AND REPORT_TYPE IN ('QUARTER','ANNUAL') ORDER BY FISCAL_YEAR DESC");
			paramMap.put("COMPANY_ID", searchObject.getCompanyId());
	...
	public String fiscalQuarter(IfoBalanceSheetSearch searchObject) throws SystemException {
		...
		try {
			Map paramMap = new HashMap();
			StringBuffer sql = new StringBuffer(
			        "SELECT DECODE(TO_CHAR(FISCAL_DATE, 'MM'),'03', 'Q1','06', 'Q2','09', 'Q3','12', 'Q4') AS FISCAL_QUARTER FROM (SELECT FISCAL_DATE FROM IFO_CASHFLOW_VIEW WHERE COMPANY_ID = :COMPANY_ID AND REPORT_TYPE = 'QUARTER'ORDER BY FISCAL_DATE DESC) WHERE ROWNUM =1");
			paramMap.put("COMPANY_ID", searchObject.getCompanyId());
