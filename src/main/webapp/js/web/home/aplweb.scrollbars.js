...
*/
(function($) {
	$.fn.scrollbars = function(_option) {
		return $(this).each(function() {
			...
			//Create touch event for smarth devices
			...
			//On page load
			touchScroll(_option.scrollId.substring(2));
			...
			function _scrollLazyLoading(){
				var scrollEnd = $scrollcontent.scrollTop() / ($scrollwrap.outerHeight() - $scrollcontent.innerHeight());
				if(scrollEnd == 1 && ($(_option.scrollId + ' .jscroll-inner').find('.jscroll-next:last').length > 0)) {
					$(_option.scrollId + ' .jscroll-inner').append('<div class="jscroll-added"></div>');
					var url = $(_option.scrollId + ' .jscroll-inner').find('a:last').attr('href');
					$(_option.scrollId + ' .ajax_loading').css("display", "block");
					$(_option.scrollId + ' .jscroll-added').load(url, function(){
						$(_option.scrollId + ' .ajax_loading').css("display", "none");
					});
					$(_option.scrollId + ' .jscroll-next-parent').remove();
				}
			}
