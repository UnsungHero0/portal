var _oscClazzChangeTeleTradeFones = new OSCClazzChangeTeleTradeFones();


$().ready(function() {
	var opts = _oscClazzChangeTeleTradeFones.getOption();
	
	$(opts.form.fields.accountNumber).change(function(){
		// set action to form
		$(opts.form.name).attr("action", CHANGE_ACCOUNT_ACTION);
		// ajax submit
		_oscClazzChangeTeleTradeFones.execute();
	});
	
	$(opts.buttons.confirm).click(function() {
		$(opts.form.fields.errorMessage).html("");
		_oscClazzChangeTeleTradeFones.init();
