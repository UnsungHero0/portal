var _oscClazzChangeInternetEmail = new OSCClazzChangeInternetEmail();


$().ready(function() {
	var opts = _oscClazzChangeInternetEmail.getOption();
	
	$(opts.form.fields.accountNumber).change(function(){
		// set action to form
		$(opts.form.name).attr("action", CHANGE_ACCOUNT_ACTION);
		// ajax submit
		_oscClazzChangeInternetEmail.execute();
	});
	
	$(opts.buttons.confirm).click(function() {
		$(opts.form.fields.errorMessage).html("");
		_oscClazzChangeInternetEmail.init();
